...
- Ensimmäinen ajokerta muuttaa tilaa --> näkyy changes kohdassa, tässä tapauksessa ei koska se oli jo valmiina ladattu

  Toiset ajokerrat --> idempotentti, koska paketin tila on jo haluttu


file komento (perus tiedostonhallinta):
...
- file.managed ja file.absent ovat idempotentteja


service komennot (palvelun hallinta): 

  
Käynnistä ja ota palvelu käyttöön, tässä tapauksessa apache2: 
...
Pysäytä ja poista palvelu käytöstä: 
  ...
  - Salt vertaa siis palvelun nykytilaa ja tavoitetilaa
    

  user komennot (käyttäjienhallinta): 
...
Poista käyttäjä
  ...
  cmd komento (komentojen suorittaminen, luo tiedoston touch komennolla): 
  ...
  - cmd.run ei ole automaattisesti idempotentti ilman creates tai unless tms.
...
Seuraavalla komennolla muokkaan kyseisen tiedoston oikeuksia:
  ...
  $ sudo salt-call --local state.single file.managed /tmp/foo user=root group=root mode=644 contents="Hei Aapo\n" -l info
